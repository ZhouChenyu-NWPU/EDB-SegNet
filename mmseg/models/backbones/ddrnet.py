...
from .model_utils import Muti_AFF, MFACB, ESPMFACB, EESPSPASPP, SPASPP
# from ..classification.model_utils import SPASPP
from ..nn_layers.eesp import EESP

import numpy as np

from ultralytics.nn.modules.mamba_yolo import VSSBlock, VisionClueMerge
#from .csp_darknet import SPPBottleneck
# class DownsampleBlock(nn.Module):
#     def __init__(self, in_channels=64, out_channels=256):
#         super().__init__()
#         mid_channels = out_channels // 2  # 先升到中间维度，如128
#
...
class DDRNet(BaseModule):
    ...
    def __init__(self,
        ...
        # # self.gltb2 = Block(dim=128, num_heads=8, window_size=8)
        ...
        self.layer3 = nn.Sequential(
                ESPMFACB(planes * 1, planes * 1, planes * 2, stride_1=2, Ker=[4, 4, 4], recept_limit=[13, 11, 9]),

        )
        #
        self.layer4 = nn.Sequential(
                ESPMFACB(planes * 2, planes * 2, planes * 4, stride_1=2, Ker=[4, 4, 4], recept_limit=[13, 11, 9]),
                # ESPMFACB(planes * 4, planes * 4, planes * 4, stride_1=1, Ker=[4, 4, 4], recept_limit=[11, 9, 7]),
                # ESPMFACB(planes * 4, planes * 4, planes * 4, stride_1=1, Ker=[4, 4, 4], recept_limit=[9, 7, 5]),
                # EESP(self.eesp_channels[3], self.eesp_channels[4], stride=2, k=self.K[1], r_lim=self.recept_limit[1]),
                # MFACB(planes * 2, planes * 2, planes * 4, stride_1=2, dilation=[3, 3, 3]),
                # MFACB(planes * 2, planes * 2, planes * 4, stride_1=2, dilation=[2, 3, 5]),
                # MFACB(planes * 4, planes * 4, planes * 4, dilation=[5, 5, 5]),
        )

        self.layer5 = EESP(self.eesp_channels[4], self.eesp_channels[3], stride=2, k=self.K[2],
                           r_lim=self.recept_limit[2])

        # self.layer5 = nn.Sequential(
        #         MFACB(planes * 4, planes * 4, planes * 8, stride_1=2, dilation=[3, 3, 3]),
        #         # MFACB(planes * 8, planes * 8, planes * 8, dilation=[5, 5, 5]),
        # )
        # self.layer5 = (MFACB(planes * 4, planes * 4, planes * 8, stride_1=2, dilation=[3, 3, 3]))
        # self.layer5 = self._make_layer(Bottleneck, inplanes=planes * 4, planes=channels * 8, num_blocks=1, stride=2)

        self.layer5_ = EESP(self.eesp_channels[2], self.eesp_channels[3], stride=1, k=self.K[2], r_lim=self.recept_limit[2], Spatial=False)
        # self.layer5_ = self._make_layer(Bottleneck, inplanes=planes * 1, planes=channels * 2, num_blocks=1, stride=1)
        # self.layer5 = ESPMFACB(planes * 4, planes * 4, planes * 8, stride_1=2, Ker=[4, 4, 4], recept_limit=[13, 11, 9])
        self.layerC3 = nn.Sequential(
            # DSESP(self.eesp_channels[2], self.eesp_channels[3], stride=2, k=self.K[2], r_lim=self.recept_limit[2])
            VisionClueMerge(dim=64, out_dim=128),
            VSSBlock(in_channels=128, hidden_dim=128)
        )
        #self.myvss = VSSBlock(in_channels=64, hidden_dim=64)
        #self.down = DownsampleBlock(in_channels=64, out_channels=256)
        self.down = nn.Sequential(
        ...
        self.sobel_y = nn.Parameter(torch.tensor([[[[-1, -2, -1],
                                    ...
                                    requires_grad=False)
        self.alpha = 0.1  # 融合比例，可学习或手动设置



        #self.channel_compress = nn.Conv2d(512, 256, 1)

        # self.layerC4 = nn.Sequential(
        #     VisionClueMerge(dim=128, out_dim=256),
        #     VSSBlock(in_channels=256, hidden_dim=256))

        # self.layerC5 = nn.Sequential(
        #     VisionClueMerge(dim=256, out_dim=128),
        #     VSSBlock(in_channels=128, hidden_dim=128))
        ...
        # self.spp_end = EESPSPASPP(512, 128, 128)
    ...
    def _make_stdc_eesp_stage(self, in_planes, out_planes, stride_1=2):
        layers = []
        layers.append(
            ESPMFACB(in_planes, in_planes, out_planes, stride_1=1, Ker=[4, 4, 4], Spatial=True, recept_limit=[13, 11, 9]))
        return Sequential(*layers)
    ...
    def _laplacian(self, x):
        ...
        laplacian_map = F.conv2d(seg_label, self.laplacian_kernel, padding=1)

        sobel_x_map = F.conv2d(seg_label, self.sobel_x, padding=1)
        sobel_y_map = F.conv2d(seg_label, self.sobel_y, padding=1)
        sobel_map = torch.sqrt(sobel_x_map ** 2 + sobel_y_map ** 2 + 1e-6)

        # 融合 Laplacian 和 Sobel
        boundary_targets = self.alpha * laplacian_map + (1 - self.alpha) * sobel_map
    ...
    def forward(self, x):
        ...
        #x_c = self.gltb1(x_c)
        x_s = self.spatial_branch_layers[0](x)+x#14gaidpgde15
        ...
        # x_c = self.gltb2(x_c)
        x_c = self.layer4(self.relu(x_c)) + self.down2(x_c)
        # UNETFormer GLTB   256*16*32
        # x_c = self.gltb2(x_c)
        x_s = self.spatial_branch_layers[1](self.relu(x_s)) + x_s#13上改动得14
