...
from ..nn_layers.efficient_pt import TransBlock, LongConnection
from ..nn_layers.eesp_down import DownSampler1, DownSampler2
...
class DDRNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.transformer_spatial_1 = TransBlock(dim=144)
        self.transformer_spatial_2 = TransBlock(dim=144)
        self.LC_1 = LongConnection(64,16,3)
        self.LC_2 = LongConnection(16,64,3)
    ...
    def forward(self, x):
        ...
        b, c, h, w = x_s.shape
        # transformer注意力
        x_s = self.transformer_spatial_1(x_s)
        x_s = x_s.permute(0, 2, 1)
        x_s = reverse_patches(x_s, (h, w), (3, 3), 1, 1)
        x_s = self.LC_2(x_s)
        ...
        # stage5
        h3 = torch.cat((h1, x_s),dim=1)
        # transformer注意力
        x_s = self.LC_1(x_s)
        b, c, h, w = x_s.shape
        # transformer注意力
        x_s = self.transformer_spatial_2(x_s)
        x_s = x_s.permute(0, 2, 1)
        x_s = reverse_patches(x_s, (h, w), (3, 3), 1, 1)
        x_s = self.LC_2(x_s)
