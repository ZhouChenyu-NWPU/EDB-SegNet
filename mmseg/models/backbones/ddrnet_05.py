...
class DDRNet(BaseModule):
    ...
    def __init__(self,
        ...
        self.stage_strides = [(2, 1), (2, 1), (2, 1)]
        ...
        # low resolution(context) branch ###原支路
        ...
        for strides in self.stage_strides:
            idx = len(self.context_branch_layers)
            self.context_branch_layers.append(
                self._make_stdc_stage(
                    self.channels[idx], self.channels[idx + 1],
                    strides, norm_cfg, act_cfg, bottleneck_type='cat')
            )
    ...
    def _make_stdc_stage(self, in_channels, out_channels, strides, norm_cfg,
