...
class ESPMFACB(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride_1=1, stride=1, Ker=[4,4,4], recept_limit=[13, 11, 9], Spatial=False, dilation=[2, 2, 2]):
        ...
        self.conv_list = nn.ModuleList()
        self.stride = stride
        self.conv_list.append(EESP(in_planes, inter_planes, stride=stride_1, k=Ker[0], Spatial=Spatial, r_lim=recept_limit[0]))
        self.conv_list.append(EESP(inter_planes, inter_planes, stride=stride, k=Ker[1], Spatial=Spatial, r_lim=recept_limit[1]))
        self.conv_list.append(EESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, r_lim=recept_limit[2]))
        self.process1 = nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride_1, padding=0, bias=False),
            BatchNorm2d(out_planes, momentum=bn_mom),
            nn.ReLU(inplace=True)
...
class EESPSPASPP(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride_1=1, stride=1, Ker=[4,4,4], recept_limit=[23, 21, 19, 17], Spatial=False, dilation=[6,12,18,24]):
        ...
        # self.conv_list.append(EESP(in_planes, inter_planes, stride=stride, k=Ker[0], Spatial=Spatial, r_lim=recept_limit[0]))
        self.conv_list.append(EESP(in_planes, inter_planes, stride=stride_1, k=Ker[0], Spatial=Spatial, SPASPP_Flag=True, r_lim=recept_limit[0]))
        self.conv_list.append(EESP(inter_planes, inter_planes, stride=stride, k=Ker[1], Spatial=Spatial, SPASPP_Flag=True, r_lim=recept_limit[1]))
        self.conv_list.append(EESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, SPASPP_Flag=True, r_lim=recept_limit[2]))
        self.conv_list.append(EESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, SPASPP_Flag=True, r_lim=recept_limit[3]))
...
class Muti_AFF(nn.Module):
    ...
    '''
    def __init__(self, channels=64, r=4, Ker=[4,4,4], recept_limit=[13, 11, 9]):
        ...
        #     # EESP(channels, inter_channels, stride=1, k=Ker[0], Spatial=False, r_lim=recept_limit[0]),
        #     # EESP(channels, inter_channels, stride=1, k=Ker[0], Spatial=False, r_lim=recept_limit[0]),
        #     nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
        #     nn.BatchNorm2d(inter_channels),
        #     nn.ReLU(inplace=True),
