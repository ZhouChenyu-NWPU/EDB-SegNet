...
def visualize_features(features, layer_name):
    ...
    # third_channel = feature_map[2]  # 选择第三个通道
    #
    # # 对特征图进行2倍上采样
    # resized_feature_map = F.interpolate(third_channel.unsqueeze(0).unsqueeze(0), scale_factor=1, mode='bilinear',
    #                                     align_corners=False).squeeze(0).squeeze(0)

    # 对所有通道进行平均
    averaged_feature_map = feature_map.mean(dim=0)  # 在通道维度上取平均

    # 对特征图进行2倍上采样
    resized_feature_map = F.interpolate(averaged_feature_map.unsqueeze(0).unsqueeze(0), scale_factor=2, mode='bilinear',
                                        align_corners=False).squeeze(0).squeeze(0)
...
class MFACB(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride_1=1, stride=1, dilation=[2, 2, 2]):
        super(MFACB, self).__init__()
        assert block_num > 1, print("block number should be larger than 1.")
        self.conv_list = nn.ModuleList()
        self.stride = stride
        self.conv_list.append(ConvX(in_planes, inter_planes, stride=stride_1, dilation=dilation[0]))
        ...
        self.process1 = nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride_1, padding=0, bias=False),
...
class CSESP(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride_1=1, stride=1, Ker=[4,4,4], recept_limit=[13, 11, 9], Spatial=False, dilation=[2, 2, 2]):
        super(CSESP, self).__init__()
        assert block_num > 1, print("block number should be larger than 1.")
        # self.K = [4, 4, 4, 4, 4]
        ...
        self.conv_list = nn.ModuleList()
        self.stride = stride
        self.conv_list.append(SESP(in_planes, inter_planes, stride=stride_1, k=Ker[0], Spatial=Spatial, r_lim=recept_limit[0]))
        self.conv_list.append(SESP(inter_planes, inter_planes, stride=stride, k=Ker[1], Spatial=Spatial, r_lim=recept_limit[1]))
        self.conv_list.append(SESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, r_lim=recept_limit[2]))
        self.process1 = nn.Sequential(
            nn.Conv2d(in_planes, out_planes, kernel_size=1, stride=stride_1, padding=0, bias=False),
            BatchNorm2d(out_planes, momentum=bn_mom),
            nn.ReLU(inplace=True)
        ...
        self.attention = ChannelAttention(out_planes)
    ...
    def forward(self, x):
        ...
        for idx in range(3):
            ...
            out_list.append(out)
        out = torch.cat(out_list, dim=1)
        return self.attention(self.process2(out) + out1)
        # return self.process2(out) + out1

class EESPSPASPP(nn.Module):
    def __init__(self, in_planes, inter_planes, out_planes, block_num=3, stride_1=1, stride=1, Ker=[4, 4, 4],
                 recept_limit=[23, 21, 19, 17], Spatial=False, dilation=[6, 12, 18, 24]):
        super(EESPSPASPP, self).__init__()
        assert block_num > 1, print("block number should be larger than 1.")
        self.conv_list = nn.ModuleList()
        self.stride = stride
        self.conv_list.append(
            SESP(in_planes, inter_planes, stride=stride_1, k=Ker[0], Spatial=Spatial, SPASPP_Flag=True,
                 r_lim=recept_limit[0]))
        self.conv_list.append(
            SESP(inter_planes, inter_planes, stride=stride, k=Ker[1], Spatial=Spatial, SPASPP_Flag=True,
                 r_lim=recept_limit[1]))
        self.conv_list.append(
            SESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, SPASPP_Flag=True,
                 r_lim=recept_limit[2]))
        self.conv_list.append(
            SESP(inter_planes, inter_planes, stride=stride, k=Ker[2], Spatial=Spatial, SPASPP_Flag=True,
                 r_lim=recept_limit[3]))
...
class MSFA(nn.Module):
    ...
    def __init__(self, channels=64, kernel_size=1, r=4, Ker=[4, 4, 4], recept_limit=[13, 11, 9]):
        super(MSFA, self).__init__()
        inter_channels = channels
        self.kernel_size = kernel_size
        self.padding = 1 if self.kernel_size > 1 else 0
        self.local_att = nn.Sequential(
            nn.Conv2d(channels, inter_channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(inter_channels),
            nn.ReLU(inplace=True),
        ...
        self.context1 = nn.Sequential(
            ...
            nn.Conv2d(channels, inter_channels, kernel_size=self.kernel_size, stride=1, padding=self.padding),
            nn.BatchNorm2d(inter_channels),
            nn.ReLU(inplace=True),
            nn.Conv2d(inter_channels, channels, kernel_size=1, stride=1, padding=0),
            nn.BatchNorm2d(channels)
    ...
    def forward(self, x, residual):
        ...
        # c4 = F.interpolate(c4, size=[h, w], mode='nearest')

        # visualize_xc_xs(c1, c2)

        xlg = xl + xg + c1 + c2 + c3
        # xlg = xl + c1 + c2 + c3

        # visualize_xc_xs(xlg, c3)
