...
from ..utils.basic_block import BasicBlock_1, BasicBlock_2
from ..nn_layers.eesp_down import DownSampler1, DownSampler2, DownSampler3
...
class PIDNet(BaseModule):
    ...
    def __init__(self,
        ...
        sc_ch_dict = {0.5: [32, 64, 128, 256, 512]}
        out_channel_map = sc_ch_dict[0.5]

        # I Branch
        self.i_branch_layers = nn.ModuleList()
        self.i_branch_layers.append(DownSampler1(out_channel_map[1], out_channel_map[2], k=4, r_lim=9, reinf=True))
        for i in range(1, 3):
        ...
        self.p_branch_layers.append(DownSampler2(out_channel_map[1], out_channel_map[1], k=4, r_lim=9, reinf=True))
        for i in range(1,3):
        ...
        self.pag_pi_1 = PagFM(channels * 4, channels*2)
        self.pag_pi_2 = PagFM(channels * 8, channels * 4)

        # D Branch
        self.d_branch_layers = nn.ModuleList()
        self.d_branch_layers.append(DownSampler3(out_channel_map[1], out_channel_map[0], k=4, r_lim=9, reinf=True))
        self.d_branch_layers.append(self._make_layer(Bottleneck, channels, channels, 1))
        self.d_branch_layers.append(self._make_layer(Bottleneck, channels * 2, channels * 2, 1))
        ...
        # self.dfm1 = dfm_module(
        #     channels * 8, channels * 8, norm_cfg=norm_cfg, act_cfg=act_cfg_dfm)

        # self.d_branch_layers.append(
        #     self._make_layer(Bottleneck, channels * 2, channels * 2, 1))

        self.basic_pi_1 = BasicBlock_1(in_channels=64,
                                       channels=128)

        self.basic_di_1 = BasicBlock_1(in_channels=32,
                                       channels=128)

        self.basic_pi_2 = BasicBlock_2(in_channels=64,
                                       channels=256)

        self.basic_di_2 = BasicBlock_2(in_channels=64,
                                       channels=256)
