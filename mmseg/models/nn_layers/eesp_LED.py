...
class SESP(nn.Module):
    ...
    def __init__(self, nIn, nOut, stride=1, k=4, r_lim=7, down_method='esp', Spatial=True, SPASPP_Flag=False, SESPV2=False):  #down_method --> ['avg' or 'esp']
        '''
        ...
        self.proj_1x1_add = CBR(nIn, n_in, 1, stride=1, groups=k)
        self.proj_1x1 = CBR(nIn, nIn, 1, stride=1, groups=k)
        self.k_sizes = list()
        if self.Spatial:
        ...
        for i in range(k):
            ...
            self.spp_dw.append(CDilated(n_in, n, kSize=3, stride=stride, groups=n, d=d_rate))
        if self.SESPV2:
            self.spp_dw_v2 = nn.ModuleList()
            for i in range(k):
                d_rate = map_receptive_ksize[self.k_sizes[i]]
                self.spp_dw_v2.append(CDilated(n_in, n, kSize=3, stride=1, groups=n, d=d_rate+1))
    ...
    def forward(self, input):
        ...
        output1_add = self.proj_1x1_add(input)
        self.mid_channel = int(output1.shape[1]*0.25)
        output = [self.spp_dw[0](output1[:, 0:self.mid_channel, ...] + output1_add)]
        # compute the output for each branch and hierarchically fuse them
        # i.e. Split --> Transform --> HFF
        for k in range(1, len(self.spp_dw)):
            out_k = self.spp_dw[k](output1[:, k*self.mid_channel:(k+1)*self.mid_channel, ...] + output1_add)
            # HFF
...
class DSESP(nn.Module):
    '''
    最新更新：2024年5月7日，增加了d_rate的判断
    This class defines the SESP block, which is based on the following principle
        REDUCE ---> SPLIT ---> TRANSFORM --> MERGE
    '''

    def __init__(self, nIn, nOut, stride=2, k=4, r_lim=7, down_method='esp', Spatial=True, SPASPP_Flag=False, SESPV2=False):  #down_method --> ['avg' or 'esp']
        '''
        :param nIn: number of input channels
        :param nOut: number of output channels
        :param stride: factor by which we should skip (useful for down-sampling). If 2, then down-samples the feature map by 2
        ...
        assert down_method in ['avg', 'esp'], 'One of these is suppported (avg or esp)'
        # assert n == n1, "n(={}) and n1(={}) should be equal for Depth-wise Convolution ".format(n, n1)
        self.proj_1x1_add = CBR(nIn, n_in, 1, stride=1, groups=k)   # 96-24
        self.proj_1x1 = CBR(nIn, nIn, 1, stride=1, groups=k)  # 96-96
        self.k_sizes = list()
        if self.Spatial:
            map_receptive_ksize = {3: 1, 5: 1, 7: 1, 9: 1, 11: 1, 13: 1, 15: 1, 17: 1}
            for i in range(k):
        ...
        for i in range(k):
            d_rate = map_receptive_ksize[self.k_sizes[i]]
            self.spp_dw.append(CDilated(n_in, n_out, kSize=3, stride=stride, groups=n_in, d=d_rate))
        if self.SESPV2:
            self.spp_dw_v2 = nn.ModuleList()
            for i in range(k):
                d_rate = map_receptive_ksize[self.k_sizes[i]]
                self.spp_dw_v2.append(CDilated(n_in, n_out, kSize=3, stride=1, groups=n_in, d=d_rate+1))
        # Performing a group convolution with K groups is the same as performing K point-wise convolutions
        self.conv_1x1_exp = CB(nOut, nOut, 1, 1, groups=k)
        self.br_after_cat = BR(nOut)
        self.module_act = nn.PReLU(nOut)
    ...
    def forward(self, input):
        ...
        # Reduce --> project high-dimensional feature maps to low-dimensional space
        output1 = self.proj_1x1(input)  # 96-96
        output1_add = self.proj_1x1_add(input)  # 96-24
        self.mid_channel = int(output1.shape[1]*0.25)
        output = [self.spp_dw[0](output1[:, 0:self.mid_channel, ...] + output1_add)]
        # compute the output for each branch and hierarchically fuse them
        # i.e. Split --> Transform --> HFF
